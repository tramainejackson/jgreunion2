                ...
                <ul class="list-group">
                    ...
                    @foreach($reunions as $reunion)
                        ...
                        <li class="list-group-item list-group-item-action reunionItem">
                            <h2 class="" data-toggle="collapse" data-parent="#reunionAccordion"
                                href="#reunionAccordion{{$loop->iteration}}" aria-expanded="true"
                                aria-controls="reunionAccordion1">{{ $reunion->reunion_city . ' ' . $reunion->reunion_year }}</h2>
                            @if($reunion->has_site == 'Y')
                                <div class="container-fluid collapse" id="reunionAccordion{{$loop->iteration}}">
                                        ...
                                        <div class="form-group col-4">
                                            ...
                                            <select class="form-control" disabled>
                                                @foreach($states as $state)
                                                    <option
                                                        value="{{ $state->state_abb }}" {{ $reunion->reunion_state == $state->state_abb ? 'selected' : '' }}>{{ $state->state_name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="form-group col-4">
                                            <label class="form-label" for="reunion_year">Year</label>
                                            <select class="form-control" disabled>
                                    ...
                                    <div class="form-row justify-content-around mb-3">
                                        <button type="button" class="btn btn-primary col-4">Registrations <span
                                                class="badge badge-light">{{ $totalRegistrations->count() }}</span>
                                            <span class="sr-only">total registrations</span>
                                        </button>

                                        <button type="button" class="btn btn-primary col-4">Committee Members <span
